...
st.sidebar.subheader(f"📅 Tasks for {today_date}")

today_tasks = st.session_state.tasks[st.session_state.tasks['Date'] == today_date]

if today_tasks.empty:
    st.sidebar.write("No tasks for today yet!")
else:
    for i, row in today_tasks.iterrows():
        col1, col2, col3, col4 = st.sidebar.columns([5,1,1,1])
        col1.markdown(f"{row['Task']} - {row['Status']}")
...
# ---------------- Main panel: All-time Task Report ----------------
