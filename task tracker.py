...
# ---------------- USER IDENTIFICATION ----------------
USER = st.text_input("Enter your username", "guest").strip()
if USER == "":
    st.warning("Please enter a username to continue.")
    st.stop()

# ---------------- Files for persistent storage ----------------
TASKS_FILE = f"tasks_{USER}.csv"
TIMER_FILE = f"timer_{USER}.csv"

# ---------------- Fresh start: ignore old tasks (optional) ----------------
...
with tab1:
    ...
    tasks_for_day = st.session_state.tasks[st.session_state.tasks['Date'] == today_date]
    ...
    if tasks_for_day.empty:
        ...
        for i, row in tasks_for_day.iterrows():
            ...
            col2.button("Done", key=f"done_{i}", on_click=lambda idx=i: mark_done(idx))
            col3.button("Not Done", key=f"notdone_{i}", on_click=lambda idx=i: mark_notdone(idx))
            col4.button("Delete", key=f"delete_{i}", on_click=lambda idx=i: delete_task(idx))
...
with tab2:
    ...
    if stop_btn:
        if st.session_state.countdown_running:
            elapsed_seconds = (init_hours*3600 + init_minutes*60 + init_seconds) - \
                              (st.session_state.countdown_h*3600 + st.session_state.countdown_m*60 + st.session_state.countdown_s)
...
# ---------------- Sidebar: Timer Report and Clear ----------------
