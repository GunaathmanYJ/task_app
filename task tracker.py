...
if st.session_state.logged_in:
    ...
    with tab4:
        ...
        if selected_group:
            ...
            task_input = st.text_input("Add Task", value=st.session_state[task_input_key], key=task_input_key)
            add_btn = st.button("➕ Add Task", key=f"group_add_task_btn_{selected_group}")
            ...
            chat_input = st.text_input("Message", value=st.session_state[chat_key], key=chat_key)
            send_btn = st.button("Send Message", key=f"send_msg_{selected_group}")
