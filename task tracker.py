...
if "current_countdown_task" not in st.session_state:
    st.session_state.current_countdown_task = "Unnamed"
...
if st.sidebar.button("⬇️ Download Task PDF"):
    pdf_file = generate_task_pdf(st.session_state.tasks)
    with open(pdf_file, "rb") as f:
        st.sidebar.download_button("Download PDF", f, file_name=pdf_file, mime="application/pdf")
...
if not st.session_state.timer_data.empty:
    st.sidebar.dataframe(st.session_state.timer_data[["Task","Target_HMS","Focused_HMS"]], use_container_width=True)
    ...
    if st.session_state.countdown_running:
        ...
        m = st.session_state.countdown_m
        s = st.session_state.countdown_s
        elapsed = (init_hours*3600 + init_minutes*60 + init_seconds) - (h*3600 + m*60 + s)
        total_seconds_today += elapsed
