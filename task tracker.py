...
with tab2:
    ...
    with col1:
        if st.button("Start Timer"):
            ...
            elif st.session_state.timer_start is not None:
    ...
    with col2:
        if st.button("Stop Timer"):
            ...
            if st.session_state.alarm_playing:
                st.session_state.alarm_playing=False
                st.markdown("""<script>var audio=document.getElementById('alarm');if(audio){audio.pause();}</script>""",unsafe_allow_html=True)
            if st.session_state.white_noise_playing:
                st.session_state.white_noise_playing=False
                st.markdown("""<script>var audio=document.getElementById('white_noise');if(audio){audio.pause();}</script>""",unsafe_allow_html=True)

    # Real-time timer + check target
    if st.session_state.timer_start:
        elapsed = datetime.now()-st.session_state.timer_start
        total_seconds = int(elapsed.total_seconds())
        hours,remainder=divmod(total_seconds,3600)
