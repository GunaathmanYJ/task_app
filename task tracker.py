...
if st.sidebar.button("🗑️ Clear All Timers"):
    st.session_state.timer_data = pd.DataFrame(columns=["Task","Target_HMS","Focused_HMS"])
    st.session_state.timer_data.to_csv(TIMER_FILE, index=False)
    st.sidebar.success("All timers cleared!")
    ...
    for i, row in tasks_for_day.iterrows():
        ...
        with cols[0]:
            if st.button(f"Done", key=f"done_{i}"):
                ...
                st.session_state.tasks.to_csv(TASKS_FILE, index=False)
        with cols[1]:
            if st.button(f"Not Done", key=f"notdone_{i}"):
...
with tab2:
    ...
    with col_h:
        init_hours = st.number_input("Hours", min_value=0, max_value=23, value=0, step=1)
    with col_m:
        init_minutes = st.number_input("Minutes", min_value=0, max_value=59, value=0, step=1)
    with col_s:
        init_seconds = st.number_input("Seconds", min_value=0, max_value=59, value=0, step=1)

    task_for_timer = st.text_input("Task name for this countdown (optional)")
    ...
    if start_btn:
        ...
        if total_seconds <= 0: st.warning("Set a time greater than 0.")
        else:
