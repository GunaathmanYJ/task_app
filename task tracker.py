...
with tab1:
    ...
    if tasks_for_day.empty:
        ...
        # Buttons for each task (clean row, one click)
...
with tab2:
    ...
    with col_h:
        init_hours = st.number_input("Hours", 0,23,0,1, key="input_hours")
    with col_m:
        init_minutes = st.number_input("Minutes", 0,59,0,1, key="input_minutes")
    with col_s:
        init_seconds = st.number_input("Seconds", 0,59,0,1, key="input_seconds")
    ...
    if st.session_state.countdown_running:
        ...
        while st.session_state.countdown_running and (h>0 or m>0 or s>0):
            display_box.markdown(f"<h1 style='text-align:center'>{h:02d}:{m:02d}:{s:02d}</h1>\n**Task:** {st.session_state.current_countdown_task}", unsafe_allow_html=True)
            time.sleep(1)
            if s>0: s-=1
            else:
                s=59
