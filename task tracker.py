    ...
    with tab4:
        ...
        if my_groups.empty:
            ...
            for _, row in my_groups.iterrows():
                ...
                if st.button(f"📂 {grp_name}", key=f"group_btn_{safe}"):
                    st.session_state.selected_group = str(grp_id)  # force string
        ...
        if st.session_state.selected_group is not None:
            ...
            for _, msg in grp_messages.iterrows():
                st.write(f"**{msg['Username']}** ({msg['Time']}): {msg['Message']}")

            chat_input = st.text_input("Type a message", key="grp_chat_input")
            if st.button("Send", key="send_grp_msg_btn") and chat_input.strip():
