    ...
    if my_groups.empty:
        ...
        for _, row in my_groups.iterrows():
            ...
            if st.button(f"📂 {grp_name}", key=f"group_btn_{safe}"):
                st.session_state.selected_group = str(grp_id)  # ensure string

    # ---------------- SELECTED GROUP DETAILS ----------------
    if st.session_state.selected_group:
        selected_group_id = str(st.session_state.selected_group)
        sel_grp_df = my_groups[my_groups["GroupID"].astype(str) == selected_group_id]
        if not sel_grp_df.empty:
            sel_grp = sel_grp_df.iloc[0]
            ...
            # --- Group Tasks ---
            st.markdown("#### Tasks")
            grp_tasks = group_tasks[group_tasks["GroupID"].astype(str)==selected_group_id]
            ...
            grp_chat_msgs = group_chat[group_chat["GroupID"].astype(str)==selected_group_id]
